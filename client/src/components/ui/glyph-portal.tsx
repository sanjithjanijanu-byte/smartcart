...
export type GlyphPortalProps = {
  ...
  interactive?: boolean;
  /** Let a user explicitly opt into the reveal even when reduced motion is preferred. */
...
export default function GlyphPortal({
  ...
  enterLabel = "Enter section", className, id, style, onProgress, motionOverride = false,
}: GlyphPortalProps) {
  ...
  const q = `:where([data-gp-root="${uid}"])`;
  ...
  useLayoutEffect(() => {
    ...
    // Unavailable requested faces are replaced by the measured system fallback above.
    ...
    const paint = (progress: number) => {
      const isStatic = (motion.matches && !motionOverride) || !browserFrameSeen || !target;
    ...
    const layout = () => {
      ...
      H = motion.matches && !motionOverride ? Math.min(viewportHeight * 0.75, 480) : viewportHeight;
      ...
      section.dataset.gpMotion = (!motion.matches || motionOverride) && browserFrameSeen && target ? "on" : "off";
  ...
  }, [text, focusChar, interactive, fontFamily, weight, length, clipId, hasFront, motionOverride]);

  return (
    <section ref={sectionRef} id={id ?? uid} data-gp-root={uid} className={className} aria-label={text}
      data-gp-motion-override={String(motionOverride)}
      ...
      <style>{`
        ...
        @media(prefers-reduced-motion:reduce){${q}:not([data-gp-motion-override="true"]) [data-gp-pin]{position:relative!important;} ${q}:not([data-gp-motion-override="true"]) [data-gp-content]{margin-top:0!important;opacity:1!important;background:var(--gp-field)!important;min-height:0;padding-block:64px;} ${q}:not([data-gp-motion-override="true"]) [data-gp-caption]{opacity:1!important;}}
        @media(prefers-reduced-motion:reduce){${q}:not([data-gp-motion-override="true"]) [data-gp-hint]{display:none;}}
      `}</style>
